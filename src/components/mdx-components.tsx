...
const components = {
    ...
    h3: ({ className, ...props }) => <h3 className={cn('mt-10 mb-2 scroll-m-20 text-2xl font-extrabold tracking-tight', className)} {...props} />,
    h4: ({ className, ...props }) => <h4 className={cn('mt-10 mb-2 scroll-m-20 text-xl font-semibold tracking-tight', className)} {...props} />,
    h5: ({ className, ...props }) => <h5 className={cn('mt-10 mb-2 scroll-m-20 text-lg font-semibold tracking-tight', className)} {...props} />,
    h6: ({ className, ...props }) => <h6 className={cn('mt-10 mb-2 scroll-m-20 text-base font-semibold tracking-tight', className)} {...props} />,
    a: ({ className, ...props }) => <a className={cn('font-medium underline underline-offset-4', className)} {...props} />,
    p: ({ className, ...props }) => <p className={cn('leading-6 [&:not(:first-child)]:mb-6', className)} {...props} />,
    ...
    blockquote: ({ className, ...props }) => <blockquote className={cn('mt-6 border-l-2 pl-6 italic [&>*]:text-muted-foreground', className)} {...props} />,
    img: ({ className, alt, ...props }: React.ImgHTMLAttributes<HTMLImageElement>) => (
        // eslint-disable-next-line @next/next/no-img-element
        <img className={cn('rounded-md border w-full', className)} alt={alt} {...props} />
    ),
    ...
    pre: ({ className, ...props }) => (
        <pre
            className={cn(
                'mb-4 mt-6 max-w-screen-xl overflow-x-auto rounded-lg border border-transparent dark:border dark:border-gray-600 bg-black p-4',
                className
            )}
            {...props}
        />
